package com.hsmart.review.infrastructure.security;
...
public class InternalSecurityFilter extends OncePerRequestFilter {
    ...
    void validateConfiguration() {
        if (enabled && !StringUtils.hasText(internalSharedSecret)) {
            throw new IllegalStateException("INTERNAL_SHARED_SECRET must be configured for review-service");
        }
    }

    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        return !request.getRequestURI().startsWith("/api/v1/reviews/internal/");
    }
