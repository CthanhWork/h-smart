...
public class InternalSecurityFilter extends OncePerRequestFilter {
    ...
    void validateConfiguration() {
        if (enabled && !StringUtils.hasText(internalSharedSecret)) {
            throw new IllegalStateException("INTERNAL_SHARED_SECRET must be configured for admin-service");
        }
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        writeResponse(response, HttpStatus.UNAUTHORIZED, UNAUTHORIZED_MESSAGE);
    }
    ...
    private void writeResponse(HttpServletResponse response, HttpStatus status, String message) throws IOException {
        ...
        response.getWriter().write("{\"status\":" + status.value() + ",\"message\":\"" + message + "\",\"data\":null}");
    }
}
