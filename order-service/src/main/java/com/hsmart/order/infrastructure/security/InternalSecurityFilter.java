package com.hsmart.order.infrastructure.security;
...
public class InternalSecurityFilter extends OncePerRequestFilter {
    ...
    void validateConfiguration() {
        if (enabled && !StringUtils.hasText(internalSharedSecret)) {
            throw new IllegalStateException("INTERNAL_SHARED_SECRET must be configured for order-service");
        }
    }

    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        return "/api/v1/orders/internal/ghtk-webhook".equals(request.getRequestURI());
    }
